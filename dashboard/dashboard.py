...
marks = {i: {'label': date.strftime('%d.%m.%Y'), 'style': {'transform': 'rotate(90deg)', 'whiteSpace': 'nowrap', 'fontSize': '10px'}}
         for i, date in enumerate(dates) if date.month in (1, 4, 7, 10) and date.day == 1}
...
def serve_layout():
    return html.Div([
        ...
        html.Div([
            ...
            html.Div([
                html.Div(dcc.Graph(id="tavg-chart"),
...
# 5) Обновление карты и топ регионов
@app.callback(
    [Output("map", "figure"), Output("top-regions", "children")],
    [Input("date-slider", "value"), Input("layer-dropdown", "value")]
)
def update_map_and_top(date_idx, layer):
    sel_date = dates[date_idx]
    dff = hist_df[hist_df["time"] == sel_date]
    fig = px.scatter_mapbox(
        dff, lat="lat", lon="lng", color=layer, size=layer,
        hover_name="city", hover_data={layer: ':.2f'},
    ...
    for idx, row in enumerate(top5.itertuples()):
        fig.add_scattermapbox(
            ...
            marker={'size': 15, 'color': top_colors[idx], 'symbol': 'square'},
            showlegend=False
        )
    items = [html.Div(
        f"{r.city}: {getattr(r, layer):.2f} {layer_names[layer].split(', ')[1]}",
        style={'backgroundColor': top_colors[i], 'padding': '8px', 'borderRadius': '10px', 'marginBottom': '5px'}
    ) for i, r in enumerate(top5.itertuples())]
    return fig, html.Div(items)

# 6) Детальные графики по региону
@app.callback(
    [Output("tavg-chart", "figure"), Output("prcp-chart", "figure"), Output("wspd-chart", "figure")],
    [Input("map", "clickData")]
)
def update_region_charts(clickData):
    if not clickData:
        empty = px.line(title="Нет данных")
        return empty, empty, empty
    city = clickData['points'][0]['hovertext']
    hist_c = hist_df[hist_df['city'] == city]
    fc = forecast_df[forecast_df['city'] == city]
    comb = pd.concat([hist_c.assign(Тип='Исторические'), fc.assign(Тип='Прогноз')])
    max_t = comb['time'].max()
    min_t = max_t - pd.Timedelta(days=438)

    # Температура: три цвета
    fig_t = px.line(
        comb, x='time', y=['tmin', 'tavg', 'tmax'],
    ...
    fig_t.add_vrect(x0=today, x1=max_t, fillcolor='lightgrey', opacity=0.3,
